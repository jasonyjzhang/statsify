    ...
    <div className={`w-full md:w-[600px] lg:w-[700px] ${loading && 'h-[100svh] flex justify-center items-center'}`}>
      {
        ...
        (
          <div className="w-full space-y-12 my-20 md:mt-28 md:mb-14">
            ...
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              ...
              {/* Track Stats */}
              <div className="relative flex flex-col items-center order-3 border-2 border-border-gray gap-y-2 md:gap-y-4 p-2 md:p-4 rounded">
                <span className="group absolute top-1 right-2">
                  <img src={InfoIcon} alt="info icon" className="w-5 inline" />
                  <span className="absolute bottom-full -translate-x-full md:left-1/2 md:-translate-x-1/2 invisible group-hover:visible">
                    <Popover text="An estimate of unique tracks listened to in the selected time range." />
                  </span>
                </span>
              ...
              {/* Artist Stats */}
              <div className="relative flex flex-col items-center order-4 border-2 border-border-gray gap-y-2 md:gap-y-4 p-2 md:p-4 rounded">
                <span className="group absolute top-1 right-2">
                  <img src={InfoIcon} alt="info icon" className="w-5 inline"/>
                  <span className="absolute bottom-full -translate-x-full md:left-1/2 md:-translate-x-1/2 invisible group-hover:visible">
                    <Popover text="An estimate of unique artists listened to in the selected time range." />
                  </span>
                </span>
            ...
            {[
              { title: 'Your Top 10 Tracks', data: userData.topTracks.tracks, isTrack: true, popoverText: 'Track popularity is based on total play count and play recency, with more recent plays weighing more.' },
              { title: 'Your Top 10 Artists', data: userData.topArtists.artists, isTrack: false, popoverText: 'Artist popularity is calculated by averaging the popularity of all their tracks.' },
            ].map(({ title, data, isTrack, popoverText }, index) => (
              <div key={index}>
                <div className="flex justify-between items-end mb-2">
                  <h2 className="text-2xl font-semibold">{title}</h2>
                  <p className={`text-dark-gray ${isTrack ? 'hidden md:inline' : 'inline'}`}>Popularity
                    <span className="group relative">
                      <img src={InfoIcon} alt="info icon" className="w-5 inline mb-1" />
                      <span className="absolute bottom-full -translate-x-full md:left-1/2 md:-translate-x-1/2 invisible group-hover:visible mb-1">
